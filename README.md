...
> should be is refused by name rather than ignored — ignoring it is the same
> failure as the missing `--config` path, and quieter. Rewrite it (the shapes
> map one to one) or delete it to run on the built-ins.
...
rules:
  - name: github-node-modules
    ...
    includes: ["**/node_modules/"]   # a trailing / is directory-only
    excludes: ["**/vendor/**"]
    requires-sibling: "*.csproj"     # a glob matching a file beside it
